function [Loc_xyz,Index]=Qin3_branch2_locGaocai(S_xyz,S_ref,tao_array,doa_ref,Qs)
 %%%情景3 释放升空散射体检测到散射体 Q>1 T=1 分支2
...
%                end
 ...
 if (Qs-1)==2
    ...
    for i=1:tdoa_size(1)
    X=X1; detaX=6;
    s1_index=tao_array(i,3);    %%在情境三下冗余
    ...
    s_xyz(:,1)=S_xyz(:,s1_index);s_xyz(:,2)=S_xyz(:,s2_index);
     ...
     dref=sqrt(S_ref(:,1)'*S_ref(:,1));
      ...
      %%%%迭代位置求解%%%%
    for n=1:20                                          %%% FPGA RD谱是以参考通道作互相关的移动项 时延计算: 其它信号-参考信号
        G(1)=-(sqrt((X-S_ref(:,1))'*(X-S_ref(:,1)))-sqrt((X-s_xyz(:,1))'*(X-s_xyz(:,1)))+dref-d1)-c*tao_array(i,1);    %%%更新G
     ...
     %%%%构建阻尼系数lamda%%%
      ...
      if(Q>0)
        ...
        if F_2<=0.5
            ...
            doa_res(2)=doa_ref(2);
            ddoa=sqrt((doa_res-doa_ref)*(doa_res-doa_ref)');
%             if (doa_ref(2)>=0 && X(3)>=0) || (doa_ref(2)<0 && X(3)<0)   %%在分支2第2种情况下不允许再对散射体Z坐标作精准定位
%                if ddoa<=7
%                 count=count+1;
%                 Index(index+1)=i;  index=index+1;
%                 Loc_xyz(:,count)=X;
...
if (Qs-1)==3
    ...
    for n=1:20                         %%% FPGA RD谱是以参考通道作互相关的移动项 时延计算: 其它信号-参考信号
        ...
        A=-[(X(1)-S_ref(1,1))/Lst_ref-(X(1)-s_xyz(1,1))/Lst,(X(2)-S_ref(2))/Lst_ref-(X(2)-s_xyz(2,1))/Lst,(X(3)-s_S_ref(3))/Lst_ref-(X(3)-s_xyz(3,1))/Lst;
            (X(1)-S_ref(1,1))/Lst_ref-(X(1)-s_xyz(1,2))/Lst2,(X(2)-S_ref(2))/Lst_ref-(X(2)-s_xyz(2,2))/Lst2,(X(3)-S_ref(3))/Lst_ref-(X(3)-s_xyz(3,2))/Lst2;
     ...
     %%%%构建阻尼系数lamda%%%
        ...
        X=X+detaX;     %%通过最小阻尼算法获得Xn+1  这里的G相当于f 即误差函数
        %%%%阻尼最小二乘的置信区域Q计算
        G_pre=G; F=0.5*(G_pre'*G_pre);                    %%% FPGA RD谱是以参考通道作互相关的移动项 时延计算: 其它信号-参考信号
        G(1)=-(sqrt((X-S_ref(:,1))'*(X-S_ref(:,1)))-sqrt((X-s_xyz(:,1))'*(X-s_xyz(:,1)))+dref-d1)-c*tao_array(i,1);    %%%更新G
        G(2)=-(sqrt((X-S_ref(:,1))'*(X-S_ref(:,1)))-sqrt((X-s_xyz(:,2))'*(X-s_xyz(:,2)))+dref-d2)-c*tao_array(i,2);
        G(3)=-(sqrt((X-S_ref(:,1))'*(X-S_ref(:,1)))-sqrt((X-s_xyz(:,3))'*(X-s_xyz(:,3)))+dref-d3)-c*tao_array(i,3);
        F_2=0.5*(G'*G);   L_cha=0.5*detaX'*(lamda*detaX-A'*G);
