...
for i=1:1
    ...
    Struct_data = mixframe_qin3(doa_endfind2,A_Sxyz,RD_cell,doaStandSca);
    %
    pf=1e-3;  flag_yundong=0;
    [StructDate,UI_printInf,flag_isUsed]=Qin3_computerservice(Struct_data,pf,flag_yundong);
   ...
   % Generate Array Signal: dir_signal + sca_signal + noise_sig 
    ...
    for i_d = 1:A_dir_num 
        ...
        for i_s=1:A_sca_num
            ...
            sca_record_array(i_s,:)=attenuation_sca(i_d,i_s)*sca_signal_N2(1,:).*exp(1i*fd_sca(i_d,i_s)*([0:N2-1]/Scample_rate*2));      
            count = 0;
            for i = 1:2:16
               ...
               sca_array_temple(count+1,:) = attenuation_sca(i_d,i_s)*sca_signal_N2(1,:).*exp(1i*fd_sca(i_d,i_s)*([0:N2-1]/Scample_rate*2)).*exp(+1i*2*pi*A_c*tao_sca(count+1)/lambda);
               count = count+1;
            end
...
%  解析帧文件生成
function Struct_data = mixframe_qin3(doa_endfind2,A_Sxyz,RD_cell,azimuth_sca)
...
%         0     0 ];
% 1目标
DOA = [doa_endfind2{1}(1) doa_endfind2{1}(2)        % 修改2 按照测向强度从高到低排列，注意将强散射体放在前面                                                                
       azimuth_sca(1,3)     azimuth_sca(2,3) 
       azimuth_sca(1,1)     azimuth_sca(1,1) 
       azimuth_sca(1,2)     azimuth_sca(2,2)   
        0     0
        0     0 
        0     0];
% 2目标
...
Sxyz = [A_Sxyz(1,1),A_Sxyz(2,1),A_Sxyz(3,1),0,azimuth_sca(1,1);
        A_Sxyz(1,2),A_Sxyz(2,2),A_Sxyz(3,2),0,azimuth_sca(1,2);
        A_Sxyz(1,3) ,A_Sxyz(2,3),A_Sxyz(3,3),0,azimuth_sca(1,3);
          0 , 0 , 0 , 0 ,0]';  
inital =21;
for i=1:4
