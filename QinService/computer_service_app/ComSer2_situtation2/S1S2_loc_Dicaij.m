function [Loc_xyz,Index]=S1S2_loc_Dicaij(S_xyz,S_varray,fd_array,doa_ref,f_array)
...
for i=1:fdoa_size(1)
X=X1;   detaX=6;
s1_index=fd_array(i,3);
s2_index=fd_array(i,4);
s_xyz(:,1)=S_xyz(:,s1_index);      %%%提取散射体坐标
s_xyz(:,2)=S_xyz(:,s2_index);

d1=sqrt(s_xyz(:,1)'*s_xyz(:,1));   
d2=sqrt(s_xyz(:,2)'*s_xyz(:,2));

s_varray(:,1)=S_varray(:,s1_index); %%%%提取散射体速度
s_varray(:,2)=S_varray(:,s2_index);

 lamda1=c/f_array(s1_index);
 lamda2=c/f_array(s2_index);  v=1;
  %%%%迭代位置求解%%%%
for n=1:20
   ...
   % Lt=sqrt(X_dis);
    ...
    G(3)=X(2)/X(1)-tan(doa_ref(1)*pi/180);
    G(4)=X(3)/sqrt(X(1:2)'*X(1:2))-tan(doa_ref(2)*pi/180);
