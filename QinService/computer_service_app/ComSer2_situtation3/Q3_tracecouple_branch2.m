function [point_cluster_cell,is_trace]=Q3_tracecouple_branch2(pufeng_cell,Q)
 %%%应用于情景3 释放升空散射体检测到一个散射体，检测点迹之间的遍历组合，分支2组合   具有散射体索引
 %%%pufeng_cell 元胞矩阵是Q>1 T=1 Q-1 个扇区数据，最大容量=3， 过滤掉全0数据，保证输入进来的数据不为0
 ...
 if (Q-1)==2
    ...
    point_cluster_cell=cell(1,1);            %%%散射体两个 一个数据空间
    point_cluster_cell{1}=zeros(shape_s1(2)*shape_s2(2),10);  %%%存放两个散射体组合数据空间 增加两个索引位 10 
...
%  
 if (Q-1)==3
    ...
    point_cluster_cell=cell(1,1);                        %%% 散射体三个 一个数据空间
...
%     row_size2=shape_s1(2)*shape_s2(2)+shape_s1(2)*shape_s3(2)+shape_s2(2)*shape_s3(2);
    ...
    point_cluster_cell{1}=zeros(row_size3,15);           %%% 存放散射体单个数据组合空间   15
