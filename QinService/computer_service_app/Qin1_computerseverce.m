...
%%% 情景一计算服务器 由于情景1与另外两种情景不会交替出现因此 在情景一下直接获得帧数
%%% Num_m    最大有效站数
%%% Num_zhen 一站获得的有效帧数
%%% tcp -> MutilState_cell  cell元组 套cell元组 内cell元组是每个站内的解析后的帧数据
%%% Loc:  获得的最终定位目标
%%% flag_isUsed =0;     %%用于判断最终数据是否是有效数据
addpath(genpath('E:\A_Matlab2020a\Matlab2020a\bin\computer_service_app\ComSer2_situtation1'))
Process_M=0;         %%已经处理的站数
XYZ_ref=[0;0;0];     %%参考站 初始为[0 0 0]
ClusterDoa_cell=cell(1,Num_m);  %%%存储 Num_m个有效站的的DOA聚类结果
count_num=0;        %%计算获得的站DOA
Loc_n=zeros(3,Num_m);
Loc_Target=zeros(4,100); %%直线交汇定位后目标点存储 默认不超过100
count_Target=1;     %%用于更新Loc_Target
UI_printInf=cell(1,3);
FrameInState_num = Num_zhen;
index = 0;
 while (true)
    %
        % 未来 tcp 获取指定站下的固定解析帧， 不判断同站内帧是否有效默认连续收集的帧有效
        % 收集完将cell数组(存储帧)依次放进MutilState_cell中
        index = index+1;
    %
     if Process_M>=1
        ...
        XYZ_ref = XYZ_next;
        CPI_DOAcell = AcquireDoA(MutilState_cell{index},FrameInState_num);        % CPI_DOAcell是元胞矩阵 第一帧数据的DOA已经进行过滤
        ClusterDoa_cell{count_num+1} = DOA_cluster(CPI_DOAcell,FrameInState_num); % 收集当前站DOA聚类结果; 
        ...
        dim=size(lineloc_res,2);                                                     % 获取直线交汇的纬度
        if dim>0
            Loc_Target(:,count_Target:count_Target+dim-1)=lineloc_res;
            count_Target=count_Target+dim;     % 充当更新Loc_Target 的索引
        end
     ...
     if Process_M==0                                                                % 当是起始站 不需要利用XYZ_ref进行比较
         ...
         CPI_DOAcell = AcquireDoA(MutilState_cell{1},FrameInState_num);              % CPI_DOAcell是元胞矩阵 第一帧数据的DOA已经进行过滤
         ClusterDoa_cell{count_num+1}=DOA_cluster(CPI_DOAcell,FrameInState_num);     % 收集当前站DOA聚类结果
         
         XYZ_ref= LBH_ref;               %% 无真实数据，坐标是站心数据时XYZ_ref ！=[0,0,0]
    ...
    if  Process_M == Num_m                       % 当前的判定方式并没有判断每个有效站位是否得到有效的测向交汇定位
        break;                                 % 而是只要送入减法聚类算法中的可能的目标矩阵不为空即可
    end
 end
% ***第二部分多站测向交汇定位 进行减法聚类 获得最终定位点***
Loc_Target(:,all(Loc_Target==0,1))=[];  

dim=size(Loc_Target);                  % 获取直线交汇的纬度
str = sprintf("情景1：%d站共获得%d组测向交汇定位结果！",Process_M,dim(2));
UI_printInf{1}=str;
if dim(2)>0
    Loc_sub=subcluster_loc(Loc_Target,Num_m,ya, yb, k_up, k_down, r);
    dim_sub=size(Loc_sub);
    if dim_sub(2)>0
        disp(['情景1状态Mention：',sprintf('%d',Process_M),'站减法定位结果完成，定位结果有效！']);
        flag_isUsed=1;
    else
        disp(['情景1状态Mention：',sprintf('%d',Process_M),'站减法定位结果完成，定位结果无效，尝试修改减法聚类算法参数！']);
        Loc_sub=0;
        flag_isUsed=0;
    end
else
       disp(['情景1状态Mention：',sprintf('%d',Process_M),'站测向交汇无有效定位结果！']);
       Loc_sub=-1;
       flag_isUsed=0;
end
...
str = sprintf("情景1：%d站共获得%d组测向聚类定位结果！",Process_M,size(loc_end,2));
UI_printInf{2}=str;

StructDate = ComponentStruct(MutilState_cell{index}{Num_zhen});  % 最后一站 最后一帧
